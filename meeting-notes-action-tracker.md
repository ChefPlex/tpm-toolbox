...
One sentence: what was this meeting trying to accomplish?

---

## Decisions Made

The most important section. Every decision should have a clear statement of what was decided, who decided it, and what evidence or reasoning drove the decision. Ambiguous decisions are the ones that get relitigated.

| # | Decision | Decision Maker | Rationale | Effective Date |
|---|---------|---------------|-----------|---------------|
| D1 | | | | |
| D2 | | | | |
| D3 | | | | |

**If no decisions were made:** Note that explicitly. "No decisions made in this meeting - discussion only" is useful information. It tells anyone reading the notes that this was an input session, not a decision session.
...
Questions that came up in the meeting but were not resolved. Every open question needs an owner responsible for getting it answered and a target date.
...
Any risks or issues that came up in the discussion that need to be logged in the RAID log.

| Item | Type | Description | Owner | RAID Log Entry? |
|------|------|-------------|-------|----------------|
| | Risk / Issue | | | Yes / No |
...
## Running Action Item Tracker

For recurring meetings, maintain a running list of all open action items below. Update status at each meeting. Close items when complete - do not delete them.

| # | Action Item | Owner | Due Date | Status | Date Closed | Notes |
|---|------------|-------|---------|--------|------------|-------|
| | | | | | | |
...
**Capture during, not after.** Notes written from memory an hour later are reconstructions, not records. Whoever is taking notes should be typing in real time - even rough notes taken live are more accurate than polished notes written later.

**One notes owner per meeting.** Rotating note-taking responsibility is fine. Shared note-taking in a shared doc is fine. No assigned owner is not fine.

**Lead with decisions, not discussion.** The people reading the notes after the meeting care most about what was decided and what they need to do. Put that first.
